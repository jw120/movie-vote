...
describe("monitor reducer", () => {
  ...
  it("NEXT with 4 in queue updates", () => {
    const s: RootData = monitor("sid", "m1", "m2", ["m3", "m4", "m5", "m6"], 5, 2);
    const a: IAction = next();
    const x: RootData = monitor("sid", "m3", "m4", ["m5", "m6", "m1"], 0, 0);
    expect(frozenRootReducer(s, a)).to.deep.equal(x);
  });
  it("NEXT with 3 in queue updates", () => {
    const s: RootData = monitor("sid", "m1", "m2", ["m3", "m4", "m5"], 5, 2);
    const a: IAction = next();
    const x: RootData = monitor("sid", "m3", "m4", ["m5", "m1"], 0, 0);
    expect(frozenRootReducer(s, a)).to.deep.equal(x);
  });
  it("NEXT with 2 in queue and A ahead updates", () => {
    const s: RootData = monitor("sid", "m1", "m2", ["m3", "m4"], 3, 2);
    const a: IAction = next();
    const x: RootData = monitor("sid", "m3", "m4", ["m1"], 0, 0);
    expect(frozenRootReducer(s, a)).to.deep.equal(x);
  });
  it("NEXT with 2 in queue and B ahead updates", () => {
    const s: RootData = monitor("sid", "m1", "m2", ["m3", "m4"], 0, 2);
    const a: IAction = next();
    const x: RootData = monitor("sid", "m3", "m4", ["m2"], 0, 0);
    expect(frozenRootReducer(s, a)).to.deep.equal(x);
  });
  it("NEXT with 2 in queue and tie updates", () => {
    const s: RootData = monitor("sid", "m1", "m2", ["m3", "m4"], 3, 3);
    const a: IAction = next();
    const x: RootData = monitor("sid", "m3", "m4", ["m1", "m2"], 0, 0);
    expect(frozenRootReducer(s, a)).to.deep.equal(x);
  });
  it("NEXT with 1 in queue and A ahead updates", () => {
    const s: RootData = monitor("sid", "m1", "m2", ["m3"], 3, 2);
    const a: IAction = next();
    const x: RootData = monitor("sid", "m3", "m1", [], 0, 0);
    expect(frozenRootReducer(s, a)).to.deep.equal(x);
  });
  it("NEXT with 1 in queue and B ahead updates", () => {
    const s: RootData = monitor("sid", "m1", "m2", ["m3"], 0, 2);
    const a: IAction = next();
    const x: RootData = monitor("sid", "m3", "m2", [], 0, 0);
    expect(frozenRootReducer(s, a)).to.deep.equal(x);
  });
  it("NEXT with 1 in queue and tie updates", () => {
    const s: RootData = monitor("sid", "m1", "m2", ["m3"], 3, 3);
    const a: IAction = next();
    const x: RootData = monitor("sid", "m3", "m1", ["m2"], 0, 0);
    expect(frozenRootReducer(s, a)).to.deep.equal(x);
  });
  it("NEXT with 0 in queue and A ahead gives winner", () => {
    const s: RootData = monitor("sid", "m1", "m2", [], 3, 2);
    const a: IAction = next();
    const x: RootData = winner("sid", null, "m1", null, null);
    expect(frozenRootReducer(s, a)).to.deep.equal(x);
  });
  it("NEXT with 0 in queue and B ahead gives winner", () => {
    const s: RootData = monitor("sid", "m1", "m2", [], 0, 2);
    const a: IAction = next();
    const x: RootData = winner("sid", null, "m2", null, null);
    expect(frozenRootReducer(s, a)).to.deep.equal(x);
  });
  it("NEXT with 0 in queue and tie updates", () => {
    const s: RootData = monitor("sid", "m1", "m2", [], 3, 3);
    const a: IAction = next();
    const x: RootData = monitor("sid", "m1", "m2", [], 0, 0);
    expect(frozenRootReducer(s, a)).to.deep.equal(x);
  });
  ...
  it("NEXT requests broadcast of REMOTE_WINNER", () => {
    const s: RootData = monitor("sid", "m1", "m2", [], 3, 4);
    const a: IAction = next();
    mock.reset();
    frozenRootReducer(s, a);
  ...
  it("REMOTE_VOTE_RECEVIED with valid A vote", () => {
    const s: RootData = monitor("sid", "m1", "m2", ["q1"], 3, 3);
    const a: IAction = remoteVoteReceived("m1");
    const x: RootData = monitor("sid", "m1", "m2", ["q1"], 4, 3);
    expect(frozenRootReducer(s, a)).to.deep.equal(x);
  });
  it("REMOTE_VOTE_RECEVIED with valid B vote", () => {
    const s: RootData = monitor("sid", "m1", "m2", ["q1"], 3, 3);
    const a: IAction = remoteVoteReceived("m2");
    const x: RootData = monitor("sid", "m1", "m2", ["q1"], 3, 4);
    expect(frozenRootReducer(s, a)).to.deep.equal(x);
  });
